...
function overrideAdminExcelLogic() {
    ...
    window.deleteSet = function(setId) {
        ...
        fetch(`/api/questions/${setId}`, { headers: { 'x-admin-pin': sessionStorage.getItem('adminAuth') || '', 'x-room-pin': sessionStorage.getItem('currentRoomPin') || new URLSearchParams(window.location.search).get('pin') || '' },
            method: 'DELETE',
...
function checkConflictsBeforePreview() {
    ...
    .then(sets => {
        let fetchPromises = sets.filter(s => s.hasData).map(s => 
            fetch(`/api/questions/${s.id}`, { headers: { 'x-admin-pin': sessionStorage.getItem('adminAuth') || '', 'x-room-pin': sessionStorage.getItem('currentRoomPin') || new URLSearchParams(window.location.search).get('pin') || '' } }).then(r => r.json()).then(data => { return {id: s.id, name: s.name, data: data.questions}; })
        );
...
window.saveParsedData = function(type) {
    ...
    fetch(`/api/questions/${setIdStr}`, { headers: { 'x-admin-pin': sessionStorage.getItem('adminAuth') || '', 'x-room-pin': sessionStorage.getItem('currentRoomPin') || new URLSearchParams(window.location.search).get('pin') || '' },
        method: 'POST',
        headers: { 
